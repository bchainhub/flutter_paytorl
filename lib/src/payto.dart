...
class Payto {
  ...
  String? get language {
    return _uri.queryParameters['lang'];
  }

  /// Sets language/locale code
  set language(String? value) {
    if (value != null) {
      // Validate the language format using regex
      if (!RegexPatterns.langRegex.hasMatch(value)) {
        throw PaytoException('Invalid language format.');
      }
      _uri = _uri.replace(
        queryParameters: {..._uri.queryParameters, 'lang': value},
      );
    } else {
      final newParams = Map<String, String>.from(_uri.queryParameters)
        ..remove('lang');
